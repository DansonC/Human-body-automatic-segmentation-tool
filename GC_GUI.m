...
fixedBG = ~roipoly(im);
%c=[70 160 300 300 160 70]
%r=[41 41 41 230 230 230];
%fixedBG =~poly2mask(c,r,size(im,1),size(im,2));
%imshow(BG);
...
k = handles.K_value; %%%%train
G = 50;              %%%%train
maxIter = 10;         
diffThreshold = 0.001;%%%train
L = GCAlgo(imd, fixedBG,k,G,maxIter, Beta, diffThreshold, handles.Processbar);
%L = GCAlgo_2(imd, fixedBG,k,G,maxIter, Beta, diffThreshold);
L = double(1 - L);

CurrRes = imd.*repmat(L , [1 1 3]);
handles.img=uint8(CurrRes);
...
img=handles.img;
filename=[handles.Pathname 'GC\' handles.Filename];
